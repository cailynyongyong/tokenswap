...
async  function  trySwap(){
    ...
    // giving ability to trade tokens with a big number. approval to move erc2token up to this maximum
    ...
    // approval to move erc2token up to this maximum
    // allowing contract to get access to swap 
    const tx = await ERC20TokenContract.methods.approve( // sets maxApproval as the allowance of allowanceTarget (spender) over the caller's tokens (Metamask)
        swapQuoteJSON.allowanceTarget, //grabbing this from json 
        maxApproval,
    )
    .send({ from: takerAddress }) // caller's tokens from metamask (in approve() function)
    .then(tx => {console.log("tx: ", tx)});

    // Perform the swap
    // Quote has all the info ready to perform the swap 
    const receipt = await  web3.eth.sendTransaction(swapQuoteJSON); // using web3 provider to send transaction 
